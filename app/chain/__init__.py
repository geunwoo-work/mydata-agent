...
for filename in os.listdir(current_dir):
    if filename.endswith(".py") and filename != "__init__.py" and filename != "abstract_chain.py":
        module_name = filename[:-3]
        module = importlib.import_module(f".{module_name}", package="chain")
        ...
        for attr_name in dir(module):
            ...
            if isinstance(attr, type) and attr.__module__ == module.__name__: # 해당 파일에서 import하지 않고 직접 선언한 클래스인 경우
                instance = attr()
                CHAIN_STORES[attr_name] = instance
